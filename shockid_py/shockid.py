...
def shockid(gridx,gridy,gridz,rog,vxg,vyg,vzg,bxg,byg,bzg,prg,ndim=2):
	...
	ds=smoothdata(rog,vxg,vyg,vzg,bxg,byg,bzg,prg,ndim=2,species,margin,smthfac)
	...
	if (ndim ==2):
		[x,y] = np.argwhere(divv < -convl)
	if (ndim ==3):
		[x,y,z] = np.argwhere(divv < -convl)
...
def divergence(ds,ndim,margin,egx,egy,egz,dx,dy,dx)
	if (ndim == 2):
