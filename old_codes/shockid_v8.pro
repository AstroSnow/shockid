...
;Local maximum
    a=abs(deriv(ronorm))
    if (a(avecyl) GE a(avecyl-1)) AND (a(avecyl) GE a(avecyl+1)) then begin
        row2=[row2,row(i)]
        col2=[col2,col(i)]
    endif  
...
;    endelse
    ...
    if ronorm(avecyl) GT ronorm(avecyl+1) then begin
        ii=avecyl+1
        while (ronorm(ii) GT ronorm(ii+1)) AND (ii LT 2*avecyl-1) do ii=ii+1
        ipre=ii+1
    endif

    if ronorm(avecyl) LT ronorm(avecyl+1) then begin
        ii=avecyl+1
        while (ronorm(ii) LT ronorm(ii+1)) AND (ii LT 2*avecyl-1) do ii=ii+1
        ipos=ii+1
    endif 

    if ronorm(avecyl) GT ronorm(avecyl-1) then begin
        ii=avecyl-1
        while (ronorm(ii) GT ronorm(ii-1)) AND (ii GT 1) do ii=ii-1
        ipre=ii-1
    endif

    if ronorm(avecyl) LT ronorm(avecyl-1) then begin
        ii=avecyl-1
        while (ronorm(ii) LT ronorm(ii-1)) AND (ii GT 1) do ii=ii-1
        ipos=ii-1
    endif 
...
c=image(alog10(abs(gradmag)),x,y,rgb_tab=0,xtitle='x',ytitle='y',dim=[970,600], position=[0.1,0.1,0.7,0.9],axis_style=2,xr=[0,1],yr=[0,1])
