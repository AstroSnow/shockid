...
position=[0.1,0.95-0.17*(i*1.0+1.0),0.9,0.95-0.17*(i*1.0)],/current,xr=[-4,4],yr=[0,10],axis_style=2,ytitle='y',aspect_ratio=0.3)

if (i eq n_elements(tarr)-1) then c.xtitle='x'
	ca=c.axes
if (i ne n_elements(tarr)-1) then ca[0].hide=1
...
ps=plot(aslow(*,0),aslow(*,1),/overplot,'.r',name='slow',sym_thick=4)
