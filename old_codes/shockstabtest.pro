...
;Clasification based on transitions
    vslowpre =mean(abs(vf(ipre:avecyl-1)/sqrt(vslow2(ipre:avecyl-1))))
    valfpre  =mean(abs(vf(ipre:avecyl-1)/sqrt(vap2(ipre:avecyl-1))))
    vfastpre =mean(abs(vf(ipre:avecyl-1)/sqrt(vfast2(ipre:avecyl-1))))
    ...
    vslowpos =mean(abs(vf(avecyl+2:ipos)/sqrt(vslow2(avecyl+2:ipos))))
    valfpos  =mean(abs(vf(avecyl+2:ipos)/sqrt(vap2(avecyl+2:ipos))))
    vfastpos =mean(abs(vf(avecyl+2:ipos)/sqrt(vfast2(avecyl+2:ipos))))
...
c=image(alog10(-diva),x,y,rgb_tab=0,xtitle='x',ytitle='y',dim=[970,600], position=[0.1,0.1,0.7,0.9],axis_style=2,aspect_ratio=16,/nodata)

xr=dblarr(n_elements(slowx))
yr=dblarr(n_elements(slowy))
...
ps=plot(xr,yr,/overplot,'.r',name='slow',thick=0.1)
...
;diva[where(divv gt 0)]=1.0/0.0
diva[where(divv gt -convl)]=1.0/0.0
;c=image(alog10(-diva),x,y,rgb_tab=0,xtitle='x',ytitle='y',dim=[970,600], position=[0.1,0.1,0.7,0.9],axis_style=2)
