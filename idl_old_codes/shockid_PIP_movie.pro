...
height = 600
fps = 1
oVid = IDLffVideoWrite('shockid_PIP_movie_MHD.mp4')
vidStream = oVid.AddVideoStream(width, height, fps)

for tread=10,100,10 do begin

;Read in the data
print,'Reading data ',tread
...
c=image(alog10(abs(gradmag)),x,y,rgb_tab=0,xtitle='x',ytitle='y',dim=[970,600], position=[0.1,0.1,0.7,0.9],axis_style=2,xr=[0,1],yr=[0,1],/buffer)
...
c.close
;c.save,'shockid_PIP_t'+strtrim(tread,1)+'_'+species+'.pdf'
