...
convl=1.0 ;Convergence threshold
...
pr=ds.pr_p(margin:egx-margin,margin:egy-margin)
...
divv=(ds.vx_p(margin+1:egx-margin+1,margin:egy-margin)-ds.vx_p(margin-1:egx-margin-1,margin:egy-margin))/(ds.x(10)-ds.x(8)) $
+(ds.vy_p(margin:egx-margin,margin+1:egy-margin+1)-ds.vy_p(margin:egx-margin,margin-1:egy-margin-1))/(ds.y(10)-ds.y(8))
...
;endfor
;In vector form (so quicker)
gradx=(ds.ro_p(margin+1:egx-margin+1,margin:egy-margin)-ds.ro_p(margin-1:egx-margin-1,margin:egy-margin))/(ds.x(10)-ds.x(8))
grady=(ds.ro_p(margin:egx-margin,margin+1:egy-margin+1)-ds.ro_p(margin:egx-margin,margin-1:egy-margin-1))/(ds.y(10)-ds.y(8))
...
;Clasification based on transitions
    vslowpre =mean(vf(ipre:avecyl-1)/sqrt(vslow2(ipre:avecyl-1)))
    valfpre  =mean(vf(ipre:avecyl-1)/sqrt(vap2(ipre:avecyl-1)))
    vfastpre =mean(vf(ipre:avecyl-1)/sqrt(vfast2(ipre:avecyl-1)))
    ...
    vslowpos =mean(vf(avecyl+2:ipos)/sqrt(vslow2(avecyl+2:ipos)))
    valfpos  =mean(vf(avecyl+2:ipos)/sqrt(vap2(avecyl+2:ipos)))
    vfastpos =mean(vf(avecyl+2:ipos)/sqrt(vfast2(avecyl+2:ipos)))
...
c=image(alog10(-diva),x,y,rgb_tab=0,xtitle='x',ytitle='y',dim=[970,600], position=[0.1,0.1,0.7,0.9],axis_style=2,xr=[0,1],yr=[0,1],/buffer)
